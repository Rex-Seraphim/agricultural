...
if (id) {
const {data} = await detail("user",id)
this.setData({
payAuth:utils.isAuthFront('user','支付')
})
...
let objClo={}
objClo['picture']= data.headportrait.split(',');
...
this.setData({
title:  this.data.detailList.headportrait,
    });
...
const resUpdate = await update('user', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["headportrait"] = data.headportrait
data["price"] = data.money
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','user');
...
async reply() {
const detailList = this.data.detailList
const res = await update("user", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','user');
