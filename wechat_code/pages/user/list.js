...
if(this.data.userAccount){
    searchForm['userAccount'] = '%' + this.data.userAccount + '%'
}
...
res = await page(`user`, searchForm);
} else {
res = await list(`user`, searchForm);
}
...
item.headportrait = item.headportrait.split(",")[0];
        return item
    })
...
wx.navigateTo({
url: `/pages/user/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/user/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 user User user ${pathname}
console.log('User  user')
isAuthObj['addAuth']   = utils.isAuth("user", "新增")
isAuthObj['delAuth']  = utils.isAuth("user", "删除")
isAuthObj['editAuth']   =utils.isAuth("user", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("user", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("user", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("user", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("user",obj)
    }else{
         resList= await list("user",obj)
    }
...
item.headportrait? item.headportrait = item.headportrait.split(",")[0].replace('upload','flie'):""
return item;
});
