...
payClick() {
let table = wx.getStorageSync("nowTable");
if (this.data.seat!=1&&this.data.address.length == 0  ) {
wx.showToast({
title: '请选择地址',
icon: "none"
})
...
let infodata = res.data
    let peopleNum =infodata.curpeople?JSON.parse(JSON.stringify(infodata.curpeople)):''
...
//收货地址
address:this.data.address[0]?.address,
tel: this.data.address[0]?.phone,
consignee: this.data.address[0]?.name,
...
//减去用户余额
let  userInfoObj=Object.assign(this.data.userInfo,{});
userInfoObj.money=(parseFloat(this.data.userInfo.money) - parseFloat(item.price * item.buynumber)).toFixed(2)
this.setData({
    userInfo:userInfoObj
})
const userpayed = await update(table, userInfoObj)
...
orderid: orderId
    ...
    data.status = '已支付'
userpayed.code == 0 && this.data.types == 3 ? data['status'] = '拼团中' : ( userpayed.code == 0?data.status = '已支付':"")
const ordersRes=       await update('orders',data)
if(item.id){
