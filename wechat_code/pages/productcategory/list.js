...
if(this.data.productcategory){
    searchForm['productcategory'] = '%' + this.data.productcategory + '%'
}
...
res = await page(`productcategory`, searchForm);
} else {
res = await list(`productcategory`, searchForm);
}
...
wx.navigateTo({
url: `/pages/productcategory/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("productcategory",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/productcategory/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/productcategory/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 productcategory Productcategory productcategory ${pathname}
console.log('Productcategory  productcategory')
isAuthObj['addAuth']   = utils.isAuth("productcategory", "新增")
isAuthObj['delAuth']  = utils.isAuth("productcategory", "删除")
isAuthObj['editAuth']   =utils.isAuth("productcategory", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("productcategory", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("productcategory", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("productcategory", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("productcategory",obj)
    }else{
         resList= await list("productcategory",obj)
    }
