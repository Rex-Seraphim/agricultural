...
if (id) {
const {data} = await detail("productcategory",id)
this.setData({
payAuth:utils.isAuthFront('productcategory','支付')
})
...
const resUpdate = await update('productcategory', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','productcategory');
...
async reply() {
const detailList = this.data.detailList
const res = await update("productcategory", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','productcategory');
