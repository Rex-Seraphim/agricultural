...
if (id) {
const {data} = await detail("discussagriculturalproductinformation",id)
this.setData({
payAuth:utils.isAuthFront('discussagriculturalproductinformation','支付')
})
...
let objClo={}
objClo['picture']= data.avatarurl.split(',');
...
const commentRes = await list("discussdiscussagriculturalproductinformation", commentData);
// 评论数增加
let detailList=this.data.detailList;
detailList.discussNumber=commentRes.data.list.length;
await update('discussagriculturalproductinformation',detailList)
...
wx.setStorageSync('tableName',"discussagriculturalproductinformation" )
wx.navigateTo({
url: `/pages/discussdiscussagriculturalproductinformation/update-and-add`,
})
...
const resUpdate = await update('discussagriculturalproductinformation', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
'cartForm.tablename': "discussagriculturalproductinformation",
})
...
wx.setStorageSync('paytable','discussagriculturalproductinformation');
...
async reply() {
const detailList = this.data.detailList
const res = await update("discussagriculturalproductinformation", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','discussagriculturalproductinformation');
